...
class SwinTransformerV2(nn.Module):
    ...
    def forward(self, x):
        ...
        return pos_output, cos_output, sin_output, width_output, x
        ...
        if isinstance(module, nn.Module):
            ...
            for param_name, param in module.named_parameters():
                if param.data is not None:
                    ...
                    print(f"\tParameter: {param_name}, Device: {device}")
                    if not "cuda" in str(device):
                        cpt += 1
                if param.grad is not None:
                    device = param.grad.device
                    print(f"\tGradient: {param_name}, Device: {device}")
    print("CPU based tensor(s) : ", cpt)
...
if __name__ == "__main__":
    ...
    model = SwinTransformerV2(in_chans=1, embed_dim=24, num_heads=[3, 6, 12, 24]).cuda()
    model.to("cuda")
    check_tensor_device(model=model)
    # summary(model, (1, 224, 224))

    """summary(model, input.shape)"""
    model = SwinTransformerV2(in_chans=1, embed_dim=48, num_heads=[1, 2, 4, 8])
    model.eval()
    sum = 0.0
